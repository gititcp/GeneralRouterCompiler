package com.luffy.routercompilerlib;

import com.google.auto.service.AutoService;
import com.luffy.routerannotationlib.AutoRouter;
...
public class RouterProcessor extends AbstractProcessor {
    ...
    private void writeComponentFile(String componentName) throws Exception {
        ...
        for (Map.Entry<String, String> entry : mStaticRouterMap.entrySet()) {
            printWriter.println("com.luffy.routerapilib.router.Router.router(\"" + entry.getKey() + "\", " + entry.getValue() + ".class);");
        }
        for (String klass : mAutoRouterList) {
            printWriter.println("if (Activity.class.isAssignableFrom(" + klass + ".class)) {");
            printWriter.println("com.luffy.routerapilib.router.Router.router(com.luffy.routerapilib.rule.concreteRule.ActivityRule.ACTIVITY_SCHEME + \"" + klass + "\", " + klass + ".class);");
            printWriter.println("}");

            printWriter.println("else if (Service.class.isAssignableFrom(" + klass + ".class)) {");
            printWriter.println("com.luffy.routerapilib.router.Router.router(com.luffy.routerapilib.rule.concreteRule.ServiceRule.SERVICE_SCHEME + \"" + klass + "\", " + klass + ".class);");
            printWriter.println("}");

            printWriter.println("else if (BroadcastReceiver.class.isAssignableFrom(" + klass + ".class)) {");
            printWriter.println("com.luffy.routerapilib.router.Router.router(com.luffy.routerapilib.rule.concreteRule.ReceiverRule.RECEIVER_SCHEME + \"" + klass + "\", " + klass + ".class);");
            printWriter.println("}");
        }
        printWriter.println("}");
        printWriter.println("}");
